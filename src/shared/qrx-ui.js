...
(function () {
   ...
   *
   * opts: { input, accept, multiple, label, extraTargets, activeClass,
  ...
  ui.sourcePicker = function sourcePicker(mount, opts = {}) {
    const CONFIRM_BYTES = opts.confirmBytes != null ? opts.confirmBytes : 64 * 1024 * 1024;
    ...
    function queue(files) {
      for (const f of files) chain = chain.then(() => open(f)).catch(() => {});
    }
    ...
    // Resolves true when the rewrite may proceed.
    function confirmRewrite(file) {
      return new Promise((resolve) => {
          ...
          confirmModal = ui.modal({
            ...
            actions: [
              ...
              { key: 'go', labelKey: 'sourcePicker.confirmGo', primary: true,
                onClick: (m) => { confirmModal._go = true; m.close(); } },
            ],
            onClose: () => {
              const go = !!confirmModal._go;
              confirmModal._go = false;
              const done = confirmModal._resolve;
        ...
        confirmModal.setTitle(undefined, 'sourcePicker.confirmTitle');
        // hint carries the file name, so it is set per call rather than once
        confirmModal.el.querySelector('.qrx-modal-hint').textContent = hint;
        confirmModal.el.querySelector('.qrx-modal-hint').hidden = false;
        confirmModal.open('[data-key="go"]');
      });
    }

    async function open(file) {
      try {
        const plan = await qrx.source.inspect(file);
        if (!plan.supported) {
          const err = new Error(qrx.i18n.t('source.unsupported', { kind: plan.label }));
          err.code = 'unsupported-format';
          err.kind = plan.kind;
          err.converterHref = opts.converterHref || null;
          throw err;
        }
        if (plan.willNormalize && CONFIRM_BYTES && file.size > CONFIRM_BYTES) {
          if (!await confirmRewrite(file)) { say(''); return null; }
        }
        const desc = await qrx.source.open(file, { kind: plan.kind, onStatus: (m) => say(m) });
        current = desc;
        say(desc.warnings.length ? desc.warnings.join(' ') : '',
            desc.warnings.length ? 'warn' : undefined);
    ...
    return {
      ...
      destroy() { if (confirmModal) confirmModal.destroy(); },
    };
  };
