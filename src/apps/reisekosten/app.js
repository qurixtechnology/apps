...
(function () {
  ...
  qrx.i18n.register('app', {
    de: {
      ...
      modeBetrag: 'Betrag', modeKm: 'Kilometer', modePauschale: 'Pauschale (Nächte)',
    ...
    en: {
      ...
      modeBetrag: 'Amount', modeKm: 'Kilometres', modePauschale: 'Flat rate (nights)',
  ...
  function persistTrips() {
    ...
    state.trips.forEach((tp) => (tp.positions || []).forEach((p) => {
      if (p.dataUrl) { delete p.dataUrl; delete p.mime; p.tooBig = true; dropped = true; }
    }));
  ...
  function posNum(i, pf, val, step) {
    return '<input class="qrx-input rk-pos-num" type="number" min="0" step="' + step + '" inputmode="decimal" data-pos="' + i + '" data-pfield="' + pf + '" value="' + esc(val) + '">';
  }
  function posModeSelect(i, kind, mode) {
    const opts = kind === 'fahrt' ? [['km', 'modeKm'], ['betrag', 'modeBetrag']]
      : kind === 'uebernachtung' ? [['pauschale', 'modePauschale'], ['betrag', 'modeBetrag']] : null;
    if (!opts) return '';
    return '<select class="qrx-select rk-pos-mode" data-pos="' + i + '" data-pfield="mode">' +
      opts.map(([v, k]) => '<option value="' + v + '"' + (mode === v ? ' selected' : '') + '>' + esc(t(k)) + '</option>').join('') + '</select>';
  }
  function posRowHtml(p, i) {
    let controls;
    if (p.kind === 'fahrt' && p.mode === 'km') {
      controls = posModeSelect(i, p.kind, p.mode) + '<span class="rk-pos-mult">' + posNum(i, 'km', p.km, '1') +
        '<span class="rk-pos-x">×</span>' + posNum(i, 'ratePerKm', p.ratePerKm, '0.01') + '<span class="rk-pos-unit">' + esc(t('posKmRate')) + '</span></span>';
    } else if (p.kind === 'uebernachtung' && p.mode === 'pauschale') {
      controls = posModeSelect(i, p.kind, p.mode) + '<span class="rk-pos-mult">' + posNum(i, 'nights', p.nights, '1') +
        '<span class="rk-pos-x">×</span>' + posNum(i, 'nightRate', p.nightRate, '0.01') + '<span class="rk-pos-unit">' + esc(t('posNightRate')) + '</span></span>';
    } else {
      controls = posModeSelect(i, p.kind, p.mode) + posNum(i, 'betrag', p.betrag, '0.01');
    }
    const viewBtn = p.dataUrl ? '<button class="qrx-btn qrx-btn-sm" data-action="view-pos" data-idx="' + i + '">' + esc(t('viewReceipt')) + '</button>' : '';
    const belegLine = p.belegName
      ? '<div class="rk-pos-beleg">' + esc(t('posReceipt')) + ': ' + esc(p.belegName) + (p.tooBig && !p.dataUrl ? ' · ' + esc(t('tooBig')) : '') + '</div>' : '';
    return '<div class="rk-pos rk-pos-' + p.kind + '" data-pos="' + i + '">' +
      '<div class="rk-pos-line1">' +
        '<select class="qrx-select rk-pos-cat" data-pos="' + i + '" data-pfield="kind">' + catOptions(p.kind) + '</select>' +
        '<input class="qrx-input rk-pos-bez" data-pos="' + i + '" data-pfield="bez" value="' + esc(p.bez || '') + '" placeholder="' + esc(t('posDescPh')) + '">' +
        viewBtn +
        '<button class="rk-icon-btn" data-action="del-pos" data-idx="' + i + '" aria-label="✕">✕</button>' +
      '</div>' +
      '<div class="rk-pos-line2"><div class="rk-pos-controls">' + controls + '</div>' +
        '<div class="rk-pos-amount" id="rk-pos-amt-' + i + '">' + esc(eur(posAmount(p))) + '</div></div>' +
      belegLine +
    '</div>';
  }
  ...
  function updatePosAmount(idx) {
    const el = $('rk-pos-amt-' + idx);
    if (el) el.textContent = eur(posAmount(state.current.positions[idx]));
  }
  ...
  function onInput(e) {
    ...
    // Kosten-Positionen
    const pfield = eln.getAttribute('data-pfield');
    if (pfield) {
      const idx = +eln.getAttribute('data-pos');
      const p = tp.positions[idx];
      if (!p) return;
      if (pfield === 'kind') { setPosKind(p, eln.value); renderPositions(); renderSummary(); }
      else if (pfield === 'mode') { p.mode = eln.value; ensurePosDefaults(p); renderPositions(); renderSummary(); }
      else if (pfield === 'bez') { p.bez = eln.value; }
      else { p[pfield] = num(eln.value); updatePosAmount(idx); renderSummary(); }
      return;
    }
