...
(function () {
  ...
  async function initFiles() {
    // Bei geöffnetem Snapshot NICHT aufräumen: hydrateState spielt Reisen + Belege
    // erst ein und lädt dann neu — vorher könnten sonst noch nicht referenzierte
    // Dateien fälschlich als „verwaist" gelöscht werden.
    if (document.getElementById('qrx-app-state')) return;
  ...
  window.qurixApp.hydrateState = async function (s) {
    ...
    for (const k of Object.keys(s)) {
      ...
      if (k.indexOf(KEY_FILE_PREFIX) === 0) {   // behaltene Datei → IndexedDB
        const id = k.slice(KEY_FILE_PREFIX.length);
        let exists = false;
        try { const rec = await idbGet(id); exists = !!(rec && rec.blob); } catch (_) {}
        if (!exists) { try { idbWrites.push(idbPut(id, dataUrlToBlob(s[k]), null)); } catch (_) {} }
      } else if (store.get(k) !== s[k]) { store.set(k, s[k]); }
    }
    try { await Promise.all(idbWrites); } catch (_) {}
    // KEIN location.reload(): direkt aus dem Speicher neu rendern. Das State-<script>
    // bleibt in der exportierten Datei erhalten — ein Reload würde sonst erneut
    // hydratisieren und in eine Endlosschleife laufen.
    try { load(); renderList(); showView('list'); } catch (_) {}
  };
